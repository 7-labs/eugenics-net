...
2. **URL structure is frozen.** Flat slugs at the site root, produced by `astro.config.mjs` → `build.format: "file"` (on-disk files stay `*.html`). Do NOT migrate to directory URLs, do NOT add trailing-slash routes, do NOT rename any existing slug. Existing URLs are the SEO asset. **Canonical/served form note (2026-06-21):** Cloudflare Pages serves these at the extensionless path (`/x.html` 308-redirects to `/x`), so `canonicalUrl()`, sitemap, rss, JSON-LD, and internal links emit the extensionless form to match what is served. This is a serving-layer decision, not a slug change — filenames and slugs are unchanged.
